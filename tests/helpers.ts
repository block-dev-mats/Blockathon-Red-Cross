...
export async function environment() {
  ...
  let network = await launchAnvil(chainPort, statePath);
  const account = privateKeyToAccount(generatePrivateKey()); // Test process only; never returned to the browser.
  const config = await deploy(network.url, account.address);
  ...
  return {
    root, dbPath, backupPath, statePath, config, client, wallet, account, frontendPort, apiUrl,
    get api() { return api; },
    ...
    restartChain: async () => { await network.stop(); network = await launchAnvil(chainPort, statePath); },
    close: async () => { await api.close(); await network.stop(); await rm(root, { recursive: true, force: true }); },
